...
// Create cascading reels for tumble mechanics
...
const boardLayout = () => ({
	x: stateLayoutDerived.mainLayout().width * 0.58,
	y: stateLayoutDerived.mainLayout().height * 0.45, // Positioned to fit cosmic frame
	anchor: { x: 0.5, y: 0.5 },
	pivot: { x: BOARD_SIZES.width / 1.5, y: BOARD_SIZES.height / 2 },
	...BOARD_SIZES,
});
...
const tumbleBoardCombined = () => {
	return stateGame.tumbleBoardBase.map((tumbleReelBase, reelIndex) => {
