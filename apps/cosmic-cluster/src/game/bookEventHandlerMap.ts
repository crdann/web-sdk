...
export const bookEventHandlerMap: BookEventHandlerMap<BookEvent, BookEventContext> = {
	...
	winInfo: async (bookEvent: BookEventOfType<'winInfo'>) => {
		console.log('[DEBUG] winInfo received:', {
			totalWin: (bookEvent as any).totalWin,
			winsCount: bookEvent.wins.length,
		});
	...
	// Cascading mechanics - tumble board after wins
	tumbleBoard: async (bookEvent: BookEventOfType<'tumbleBoard'>) => {
		...
		// Settle board with combined tumble board state
		const combinedBoard = stateGameDerived.tumbleBoardCombined();
		eventEmitter.broadcast({
			type: 'boardSettle',
			board: combinedBoard.map((r) => r.map((s) => s.rawSymbol)),
		});
		eventEmitter.broadcast({ type: 'tumbleBoardReset' });
		eventEmitter.broadcast({ type: 'tumbleBoardHide' });
		eventEmitter.broadcast({ type: 'boardShow' });
	},
	// Update multiplier grid display
	...
	// Update running tumble win amount
